...
- 도서 5종은 실제 목록이고 소개문도 실제 내용입니다. 다만 03·04(컬러링 북)의 ISBN·발행년도는
  아직 확인하지 못해 `.meta` 행을 넣지 않았습니다.
- 02 『비즈니스 엑셀』은 표지가 없습니다 — 표지를 구할지 카드를 뺄지 아직 정해지지 않았습니다.
  05 『비즈니스 엑셀 & 파워포인트』가 나오면서 **02를 목록에 남길지도 같이 정해야 합니다**
  (두 책은 ISBN이 다른 별개의 책이지만 카드에서는 제목이 거의 같아 보입니다).
- 도서 카드의 카페 링크는 01~04만 실제 글 주소이고, **05는 카페 메인**을 가리킵니다 —
  새 책의 카페 글 주소를 확인하면 `cafe.naver.com/minholicbook/<번호>` 로 바꿔 주세요.
